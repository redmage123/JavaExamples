...
class MyStack <T extends Object> implements StackInterface<T> {
	...
	/*
	  Here we implement push.  It's not complete because we haven't done exception
	  handling yet.  So, there is a possibility that this could crash. 
	 */
	public boolean push(T item) {
		...
		// We should catch an exception here, but we haven't gotten to that yet...
		this.elements.add(item);
		this.top++;
		return true;
	}
	/* 
	 Here we implement pop.  Note the return value is defined as a generic type. 
	 */
	public T pop() {
		// Get the top element and pop it off of the ArrayList.
		T result = this.elements.get(this.top);
		
		// Modify the top pointer value appropriately to reflect the new stack size.
		this.top--;
	...
	/*
	 Simple method returning a boolean to test whether or not the stack is empty. 
	 We aren't using it in the app, but it's an example of a method that may very well
	 be part of a proper stack interface definition. 
	 */
